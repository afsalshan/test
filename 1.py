...
def init_pop():
    ...
    supp = [0, "Supplier", "Manufacturer", "Distributer", "Retailer"]
    ...
    for i in range(4, 0, -1):  # sum of lead time
        ...
        for j in range(i, 5, 1):
            sum = sum + int(LT[4 - j])
        sLT.append(int(sum))
    for i in range(1, 5, 1):  # holding and backlog cost
        h.append(int(input("Holding Cost for" + str(supp[5 - i]) + " ")))
        b.append(int(input("Backlog Cost for" + str(supp[5 - i]) + " ")))
